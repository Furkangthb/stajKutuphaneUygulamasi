...
export default function ManageUsersPage() {
  ...
  const [editForm, setEditForm] = useState({ username: "", email: "", role: "user" as "user" | "admin" });
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await api.updateUser(editing.id, editForm);
      setUsers((us) => us.map((u) => (u.id === editing.id ? { ...u, ...editForm } : u)));
  ...
  const del = async (user: User) => {
    if (!confirm(`"${user.username}" kullanıcısını silmek istediğinizden emin misiniz?`)) return;
    setDeleting(user.id);
    try {
                  ...
                  >
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold" style={{ color: "var(--foreground)" }}>{user.username}</span>
                      </div>
                    </td>
              ...
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    value={editForm[key as "username" | "email"]}
